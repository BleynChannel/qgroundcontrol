...
VideoManager::setToolbox(QGCToolbox *toolbox)
{
   ...
   const QString anotherVideoSource = _videoSettings->anotherVideoSource()->rawValue().toString();
   qCDebug(VideoManagerLog) << "New Video Source:" << videoSource;
   qCDebug(VideoManagerLog) << "New Another Video Source:" << anotherVideoSource;
   connect(_videoSettings->videoSource(),   		&Fact::rawValueChanged, this, &VideoManager::_videoSourceChanged);
   connect(_videoSettings->anotherVideoSource(),	&Fact::rawValueChanged, this, &VideoManager::_anotherSourceChanged);
   ...
   connect(_videoSettings->anotherUdpPort(),       	&Fact::rawValueChanged, this, &VideoManager::_anotherSourceChanged);
   connect(_videoSettings->anotherRtspUrl(),		&Fact::rawValueChanged, this, &VideoManager::_anotherSourceChanged);
   connect(_videoSettings->anotherTcpUrl(),			&Fact::rawValueChanged, this, &VideoManager::_anotherSourceChanged);
   ...
   connect(pVehicleMgr, &MultiVehicleManager::activeVehicleChanged, this, &VideoManager::_setActiveVehicle);
	...
	if (_videoReceiverData[2].receiver != nullptr) {
        (void) connect(_videoReceiverData[2].receiver, &VideoReceiver::onStartComplete, this, [this](VideoReceiver::STATUS status) {
            if (status == VideoReceiver::STATUS_OK) {
                _videoReceiverData[2].started = true;
                if (_videoReceiverData[2].sink != nullptr) {
                    _videoReceiverData[2].receiver->startDecoding(_videoReceiverData[2].sink);
                }
            } else if (status == VideoReceiver::STATUS_INVALID_URL) {
                // Invalid URL - don't restart
            } else if (status == VideoReceiver::STATUS_INVALID_STATE) {
        ...
        (void) connect(_videoReceiverData[1].receiver, &VideoReceiver::onStopComplete, this, [this](VideoReceiver::STATUS) {
            _videoReceiverData[2].started = false;
            _startReceiver(2);
        });
    }
...
void
VideoManager::_anotherSourceChanged()
{
    (void) _updateSettings(2);
    emit hasVideoChanged();
    emit isStreamSourceChanged();
    emit isUvcChanged();
    emit isAutoStreamChanged();
...
VideoManager::_initVideo()
{
    ...
    const QStringList widgetTypes = {"videoContent", "thermalVideo", "anotherVideo"};
    for (VideoReceiverData &videoReceiver : _videoReceiverData) {
...
VideoManager::_updateSettings(unsigned id)
{
    ...
    if(_activeVehicle && _activeVehicle->cameraManager()) {
        ...
        if(pInfo) {
            if (id == 0) {
				...
				qCDebug(VideoManagerLog) << "Configure another stream:" << pInfo->uri();
                switch(pInfo->type()) {
                    case VIDEO_STREAM_TYPE_RTSP:
                        if ((settingsChanged |= _updateVideoUri(id, pInfo->uri()))) {
                            _toolbox->settingsManager()->videoSettings()->anotherVideoSource()->setRawValue(VideoSettings::videoSourceRTSP);
                        }
                        break;
                    case VIDEO_STREAM_TYPE_TCP_MPEG:
                        if ((settingsChanged |= _updateVideoUri(id, pInfo->uri()))) {
                            _toolbox->settingsManager()->videoSettings()->anotherVideoSource()->setRawValue(VideoSettings::videoSourceTCP);
                        }
                        break;
                    case VIDEO_STREAM_TYPE_RTPUDP:
                        if ((settingsChanged |= _updateVideoUri(
                                        ...
                                        pInfo->uri().contains("udp://")
                                            ? pInfo->uri() // Specced case
                                            : QStringLiteral("udp://0.0.0.0:%1").arg(pInfo->uri())))) {
                            _toolbox->settingsManager()->videoSettings()->anotherVideoSource()->setRawValue(VideoSettings::videoSourceUDPH264);
                        }
                        break;
                    case VIDEO_STREAM_TYPE_MPEG_TS:
                        if ((settingsChanged |= _updateVideoUri(id, QStringLiteral("mpegts://0.0.0.0:%1").arg(pInfo->uri())))) {
                            _toolbox->settingsManager()->videoSettings()->anotherVideoSource()->setRawValue(VideoSettings::videoSourceMPEGTS);
                        }
                        break;
                    default:
                        settingsChanged |= _updateVideoUri(id, pInfo->uri());
	...
	const QString anotherSource = _videoSettings->anotherVideoSource()->rawValue().toString();
    if (anotherSource == VideoSettings::videoSourceUDPH264)
        settingsChanged |= _updateVideoUri(1, QStringLiteral("udp://0.0.0.0:%1").arg(_videoSettings->anotherUdpPort()->rawValue().toInt()));
    else if (anotherSource == VideoSettings::videoSourceUDPH265)
        settingsChanged |= _updateVideoUri(1, QStringLiteral("udp265://0.0.0.0:%1").arg(_videoSettings->anotherUdpPort()->rawValue().toInt()));
    else if (anotherSource == VideoSettings::videoSourceMPEGTS)
        settingsChanged |= _updateVideoUri(1, QStringLiteral("mpegts://0.0.0.0:%1").arg(_videoSettings->anotherUdpPort()->rawValue().toInt()));
    else if (anotherSource == VideoSettings::videoSourceRTSP)
        settingsChanged |= _updateVideoUri(1, _videoSettings->anotherRtspUrl()->rawValue().toString());
    else if (anotherSource == VideoSettings::videoSourceTCP)
        settingsChanged |= _updateVideoUri(1, QStringLiteral("tcp://%1").arg(_videoSettings->anotherTcpUrl()->rawValue().toString()));
    else if (anotherSource == VideoSettings::videoSource3DRSolo)
        settingsChanged |= _updateVideoUri(1, QStringLiteral("udp://0.0.0.0:5600"));
    else if (anotherSource == VideoSettings::videoSourceParrotDiscovery)
        settingsChanged |= _updateVideoUri(1, QStringLiteral("udp://0.0.0.0:8888"));
    else if (anotherSource == VideoSettings::videoSourceYuneecMantisG)
        settingsChanged |= _updateVideoUri(1, QStringLiteral("rtsp://192.168.42.1:554/live"));
    else if (anotherSource == VideoSettings::videoSourceHerelinkAirUnit)
        settingsChanged |= _updateVideoUri(1, QStringLiteral("rtsp://192.168.0.10:8554/H264Video"));
    else if (anotherSource == VideoSettings::videoSourceHerelinkHotspot)
        settingsChanged |= _updateVideoUri(1, QStringLiteral("rtsp://192.168.43.1:8554/fpv_stream"));
    else if (anotherSource == VideoSettings::videoDisabled || anotherSource == VideoSettings::videoSourceNoVideo)
        settingsChanged |= _updateVideoUri(1, "");
    else {
        settingsChanged |= _updateVideoUri(1, "");
        if (!isUvc()) {
            qCCritical(VideoManagerLog)
                << "Video another source URI \"" << source << "\" is not supported. Please add support!";
        }
    }
