...
DECLARE_SETTINGGROUP(Video, "Video")
{
    ...
    _nameToMetaDataMap[anotherVideoSourceName]->setEnumInfo(videoSourceCookedList, videoSourceList);
...
void VideoSettings::_setDefaults()
{
    if (_noVideo) {
        ...
        _nameToMetaDataMap[anotherVideoSourceName]->setRawDefaultValue(videoSourceNoVideo);
    } else {
        _nameToMetaDataMap[videoSourceName]->setRawDefaultValue(videoDisabled);
        _nameToMetaDataMap[anotherVideoSourceName]->setRawDefaultValue(videoDisabled);
    }
}
...
DECLARE_SETTINGSFACT_NO_FUNC(VideoSettings, anotherVideoSource)
{
    if (!_anotherVideoSourceFact) {
        _anotherVideoSourceFact = _createSettingsFact(anotherVideoSourceName);
        //-- Check for sources no longer available
        if(!_anotherVideoSourceFact->enumValues().contains(_anotherVideoSourceFact->rawValue().toString())) {
            if (_noVideo) {
                _anotherVideoSourceFact->setRawValue(videoSourceNoVideo);
            } else {
                _anotherVideoSourceFact->setRawValue(videoDisabled);
            }
        }
        connect(_anotherVideoSourceFact, &Fact::valueChanged, this, &VideoSettings::_configChanged);
    }
    return _anotherVideoSourceFact;
}
...
DECLARE_SETTINGSFACT_NO_FUNC(VideoSettings, anotherUdpPort)
{
	if (!_anotherUdpPortFact) {
		_anotherUdpPortFact = _createSettingsFact(anotherUdpPortName);
		connect(_anotherUdpPortFact, &Fact::valueChanged, this, &VideoSettings::_configChanged);
	}
	return _anotherUdpPortFact;
}

DECLARE_SETTINGSFACT_NO_FUNC(VideoSettings, anotherRtspUrl)
{
	if (!_anotherRtspUrlFact) {
		_anotherRtspUrlFact = _createSettingsFact(anotherRtspUrlName);
		connect(_anotherRtspUrlFact, &Fact::valueChanged, this, &VideoSettings::_configChanged);
	}
	return _anotherRtspUrlFact;
}

DECLARE_SETTINGSFACT_NO_FUNC(VideoSettings, anotherTcpUrl)
{
	if (!_anotherTcpUrlFact) {
		_anotherTcpUrlFact = _createSettingsFact(anotherTcpUrlName);
		connect(_anotherTcpUrlFact, &Fact::valueChanged, this, &VideoSettings::_configChanged);
	}
	return _anotherTcpUrlFact;
}
